...
def load_wav(full_path):
    ...
    if sampling_rate != 22050:
        data = resample(data, int(len(data) * 22050 / sampling_rate))
    return data, 22050
...
class MelDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, audio_start, segment_size, n_fft, num_mels,
    ...
    def __getitem__(self, index):
        ...
        if self._cache_ref_count == 0:
            cover_audio, sampling_rate = load_wav(cover)
            audio = cover_audio / MAX_WAV_VALUE
            audio = normalize(audio) * 0.95

            serect_audio, sampling_rate = load_wav(serect)
        ...
        if not self.fine_tuning:
            if self.split:
                if audio.size(1) >= self.segment_size:
                    ...
                    audio = audio[:, self.audio_start:self.audio_start + self.segment_size]
                else:
                ...
                if serect_audio.size(1) >= self.segment_size:
                    ...
                    # audio_start = random.randint(0, max_audio_start)
                    serect_audio = serect_audio[:, self.audio_start:self.audio_start + self.segment_size]
                else:
