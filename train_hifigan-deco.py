...
def train(rank, a, h):
    ...
    def stego_mel(x_mel, y_mel):
        config_file = '/cp_hifigan/hifigan/ashell3_22050/config.json'
        with open(config_file) as f:
            data = f.read()
        json_config = json.loads(data)
        h = AttrDict(json_config)
        generator = Gen(h).to(device)
        checkpoint_file = '/home/lgd/miniconda3/envs/all/Mel_in/cp_hifigan/LJ_V1/g_00780000'
        ...
        x = generator(x_mel, y_mel)
        x_hat_mel = mel_spectrogram(x.squeeze(1), h.n_fft, h.num_mels, h.sampling_rate,
                                    h.hop_size, h.win_size, h.fmin, h.fmax_for_loss)
        y_hat_mel = model(x_hat_mel)
        return y_hat_mel
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            if rank == 0:
                start_b = time.time()
            x_mel, x_aud, x_mel_loss, y_mel, y, y_mel_s = batch
...
def main():
    ...
    parser.add_argument('--checkpoint_path',
                        default='/home/lgd/miniconda3/envs/all/Mel_in/cp_hifigan/hifigan_lj')#/home/lgd/miniconda3/envs/all/Mel_in/hifigan/new_hifigan/aish_hifiga
    parser.add_argument('--config',
                        default='/home/lgd/miniconda3/envs/all/Mel_in/cp_hifigan/hifigan_lj/config.json')
